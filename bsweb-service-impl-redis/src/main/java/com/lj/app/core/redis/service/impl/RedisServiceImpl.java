...
public class RedisServiceImpl implements RedisService {
  ...
  public Boolean hset(String key, Object value) {
    /*
     * HashOperations hashOps = redisTemplate.opsForHash(); Class<?> clazz = value.getClass(); PropertyDescriptor pd =
     * null; Method method = null; Object obj = null; int count = 0; Field[] fields = ClassUtil.getClassAllFields(clazz,
     * BaseEntity.class); Set<String> fieldNameSet = new HashSet<String>(); String fieldName = null; for (Field field :
     * fields) { fieldName = field.getName(); if (!fieldNameSet.add(fieldName) || fieldName.equals("serialVersionUID"))
     * { continue; } try { pd = new PropertyDescriptor(fieldName, clazz); method = pd.getReadMethod(); if (method ==
     * null) continue; obj = method.invoke(value); hashOps.put(key, fieldName, obj); } catch (Exception e) {
     * log.error(e.getMessage(), e); continue; } count++; }
     * 
     * return count > 0;
     */
