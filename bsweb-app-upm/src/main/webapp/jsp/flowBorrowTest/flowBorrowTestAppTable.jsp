	...
	var TableInit = function () {
        ...
        oTableInit.Init = function () {
			...
			{ field: 'checkStatus', title: '',checkbox:true }, 
			 	...
			 	{field:'operatorAmount',title:'借款金额',footerFormatter:sumFloatFormatter, sortable:true, editable: true,
			 		edit:{type:'text',validate: function (value) { //字段验证
			 			 if (!v) return '借款金额不能为空';
	            	}
			 	}},
             	...
             	onEditableSave: function (field, row, oldValue, $el) {
                    ...
                    var postData = {
                    		"flowBorrowTest.id":row.id,
                    		"flowBorrowTest.processId":"${param.processId}",
            				"flowBorrowTest.orderId":"${orderId}",
            				"flowBorrowTest.taskId":"${taskId}",
            				"flowBorrowTest.operatorAmount":row.operatorAmount,
            				"flowBorrowTest.appMemo":row.appMemo
                        };
                    
                    var result = jQuery.ajax({
      		      	  url:"${ctx}/jsp/flowBorrowTest/flowBorrowTestAction!ajaxSave.action",
      		          async:false,
      		          cache:false,
      		          type:"post",
      		          dataType:"json",
       ...
       $("#btn_submit").click(function() {
    	   var postData = {
    	 	...
    	 	bootbox.confirm('您确认要提交吗?提交成功后，网页将再5秒内自动关闭',function (result) {  
	               if(result) {  
		           var result = jQuery.ajax({
				      	  url:"${ctx}/jsp/flowBorrowTest/flowBorrowTestAction!flowBorrowTestAppTableSubmit.action",
				          async:false,
				          cache:false,
				          type:"post",
				          dataType:"json",
