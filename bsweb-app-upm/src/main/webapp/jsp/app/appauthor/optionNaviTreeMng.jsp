	...
	<head>
		...
		$(function(){
			...
			if(root == null || root == ''){
				...
				if(selectName == null || selectName == ''){
					$("#treeNodeId").val(root.id);
					$("#treeNodeName").val(root.text);
				...
				tree = new TreePanel({
					...
					'switchType' : 'ajax',
					'ajaxAction' : "${ctx}/jsp/app/appauthor/commonappauthornavitreemng!list.action?appCode=${appCode}&optype=${optype}&adminAcctSeq=${adminAcctSeq}&appId=${appId}"
				});
				tree.render();
				tree.on(function(node){
					...
					if(optype == "${optype}"){
						if(selectName == null || selectName == ''){
							...
							$("#showdiv").show();
							$("#.buttongrant").removeAttr("disabled");
							$("#.tabgrant").removeAttr("disabled");
						}else{
							if(isFunction){
						...
						if(selectName == null || selectName == ''){
							$("#.buttongrant").attr("disabled",true);
							$("#.tabgrant").attr("disabled",true);
							$("#treeNodeId").val(treeNodeId);
							$("#treeNodeName").val(treeNodeName);
							add();
							dirChange(treeNodeId,node.getPath("text"));
						}
					}
				});
			}
		...
		function deleteNode(){
			...
			}else if(confirm('<s:text name="appentity.makeSureToDel"/>')){
				var isAuthor  = $.ajax({
				 	type: "POST",
					...
					<div>
						<table width="100%" height="400" id="heightTable" border="1">
							<tr height="100%">
								...
								<c:if test="${empty selectName}">
								<td width="75%" valign="top" height="98%">
									<div class="contain_search" style="width:100%;height: 100%;overflow: auto">
										<table>
											<tr>
												<td>
													<s:text name="appentity.currentDirNode"/>
												</td>
												<td>
										...
										<form
											...
											<table width="95%" cellpadding="0" cellspacing="0"
												...
												<c:forEach var="appAuthorTableConfig" items="${columnList}"
													varStatus="status">
													<c:if test="${appAuthorTableConfig.columnName==parentName}">
														<tr>
